...
// InteriorConstructionPhotos array lai import gareko
import { InteriorConstructionPhotos } from '../Comp/Data'; 
...
const InteriorConstruction = () => {
  ...
  const currentItems = InteriorConstructionPhotos.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(InteriorConstructionPhotos.length / itemsPerPage);
    ...
    <section className="py-5 bg-white mt-5">
      <div className="container py-4">
        <div className="text-center mb-5">
          <h2 className="fw-bold" style={{ color: '#001A7E' }}>Interior <span style={{ color: '#f39c12' }}>Construction</span></h2>
          <div className="mx-auto mt-2" style={{ width: '50px', height: '3px', background: '#f39c12' }}></div>
        </div>

        {/* --- Image Grid Mapping InteriorConstructionPhotos --- */}
          ...
          {currentItems.map((item) => (
            <div key={item.id} className="col-lg-3 col-md-4 col-sm-6">
              ...
              >
                <img 
                  src={item.image} 
                  alt={`Interior Construction ${item.id}`} 
