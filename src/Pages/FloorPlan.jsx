...
import { floorPlanPhotos } from '../Comp/Data'; // Path properly checked
...
const FloorPlan = () => {
  ...
  const currentItems = floorPlanPhotos.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(floorPlanPhotos.length / itemsPerPage);
    ...
    <section className="py-5 bg-white mt-5">
      <div className="container py-4">
        <div className="text-center mb-5">
          <h2 className="fw-bold" style={{ color: '#001A7E' }}>Featured <span style={{ color: '#f39c12' }}>Floor Plans</span></h2>
          ...
          {currentItems.map((item) => (
            <div key={item.id} className="col-lg-3 col-md-4 col-sm-6">
              <motion.div 
                ...
                onClick={() => setSelectedImg(item.image)} // Click garda modal khulcha
                className="rounded-4 overflow-hidden shadow-sm border"
                style={{ cursor: 'pointer', height: '250px' }}
              >
                <img src={item.image} alt="Plan" className="w-100 h-100" style={{ objectFit: 'cover' }} />
              </motion.div>
            </div>
          ))}
        </div>
      ...
      {/* --- Lightbox Modal (Click garepachi khulne) --- */}
      <AnimatePresence>
        {selectedImg && (
          <motion.div 
          ...
          >
            ...
            >
              ...
              <img src={selectedImg} className="img-fluid rounded-3 w-100" alt="Preview" />
